...
class HealthGuard {
  ...
  async validateRLSCompliance(stagedFiles) {
    ...
    for (const file of stagedFiles) {
      ...
      // Skip exempt files
      const isExempt = this.rlsManifest.exempt_operations.some(exempt =>
      ...
      for (const entity of this.rlsManifest.clinical_entities) {
      ...
      if (content.includes('JOIN') && this.rlsManifest.clinical_entities.some(e => content.includes(e))) {
